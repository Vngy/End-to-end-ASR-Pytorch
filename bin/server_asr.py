...
class Solver(BaseSolver):
    ...
    def load_data(self):
        ...
        self.dv_set, self.feat_dim, self.vocab_size, self.tokenizer = \
            load_dataset(self.paras.njobs, self.paras.gpu,
                         self.paras.pin_memory, False, **self.config['data'])
        self.verbose("Loading Incoming Data")
    ...
    def eval(self):
        for data in self.dv_set:
            self.verbose("Performing CTC Decoding on Input Audio")
            ...
            results = ctc_beam_decode_func(data)
            hyps = self.print_hyps(results)
            self.verbose("Hypothesis after decoding: {}".format(hyps))
            self.verbose('All done Decoding!')
            return hyps
    ...
    def print_hyps(self, results):
        print(results)
        hypothesis = []
        results = results[1:-1]
        for hyp_seqs in tqdm(results):
            print(hyp_seqs)
            for b, hyp in enumerate(hyp_seqs):
                print(b, '\n', hyp)
                hyp = hyp.outIndex
                hypothesis.append(self.tokenizer.decode(hyp))
            #for hyp in tqdm(hyp_seqs):
            #    print(hyp)
            #    hypothesis = self.tokenizer.decode(hyp)
        return hypothesis

    def write_hyp_server(self, results):
        ...
        for hyp_seqs in tqdm(results[1:-1]):
            print(hyp_seqs)
            if self.ctc_only and not self.greedy:
                ...
                hyp_seqs = [self.tokenizer.decode([hyp.outIndex]) for hyp in hyp_seqs]
                print(hyp_seqs)
