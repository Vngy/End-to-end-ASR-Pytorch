...
def read_text(file):
    ...
    print(idx)
    with open(transcription_dir+'/'+idx+'.txt') as fp:
        for line in fp:
            print(line)
            return line


class IncomingAudioDataset(Dataset):
    def __init__(self, path, split, tokenizer, bucket_size, ascending=False):
        ...
        self.file_list = [i.absolute() for i in file_list]
...
class IncomingAudioTextDataset(Dataset):
